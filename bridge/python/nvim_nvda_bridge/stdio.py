...
class StdioTransport:
	...
	def start(self) -> None:
		...
		self._reader = threading.Thread(
			target=self._read_controls, name="nvim-nvda-stdio-reader", daemon=True
		)
		self._heartbeat = threading.Thread(
			target=self._send_heartbeats, name="nvim-nvda-stdio-heartbeat", daemon=True
		)
		self._reader.start()
			...
			while not self._stop.is_set():
				...
				for control in decoder.feed(data):
					...
					if kind == "requestFullState":
						...
						self.on_control(kind, dict(control["payload"]))
					elif kind == "leaveTerminalInputRequest" and valid_leave_terminal_input_request(
						control.get("payload")
					):
						self.on_control(kind, dict(control["payload"]))
					elif (
						kind == "exploreTextRequest"
						and valid_explore_text_request(control.get("payload"))
						and self._supports_plugin_capability("exploration")
					):
						self.on_control(kind, dict(control["payload"]))
					elif (
						kind == "endExplorationRequest"
						and valid_end_exploration_request(control.get("payload"))
						and self._supports_plugin_capability("exploration")
					):
						self.on_control(kind, dict(control["payload"]))
	...
	def _supports_plugin_capability(self, capability: str) -> bool:
		return self._payload_supports(self._full_state(), capability)
