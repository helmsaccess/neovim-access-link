...
`CHANGE`, `COPY`, `CREATE`, `DELETE` und `MOVE` und veröffentlicht lediglich
Aktion, Anzahl sowie „Y yes, N no“ als `promptOpened`. Dateinamen, Pfade und
rohe Zeile werden nicht übertragen. Beim Verlassen oder Schließen folgt
`promptClosed`.
...
- Absicherung: Vor einer Freigabe muss ein isolierter echter Oil-Test Öffnen,
  Ablehnen, genau ein Öffnen/Schließen, fehlende Rohereignisse und unveränderte
  Testdateien belegen.
