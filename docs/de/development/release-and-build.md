...
- vom Benutzer bestimmte Produktversion `0.90.0`,
...
Produktversion `0.90.0` für `manifest.ini` und den NVDA Add-on Store.
`buildVars.development_version()` ergänzt für Entwicklungsstände eine
branchlokale SemVer-Kennung wie `0.90.0-dev.1` und nach Möglichkeit
...
- einen eindeutigen Archivnamen wie
  `nvimNvdaAccess-0.90.0-dev.1+feature.addon-isolation.<commit>.nvda-addon`,
- den sichtbaren Komponentenpaketnamen
  `neovim-access-link-0.90.0-dev.1+feature.addon-isolation.<commit>-user.tar.gz`,
