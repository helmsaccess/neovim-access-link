...
- vom Benutzer bestimmte Produktversion `0.95.0`,
...
Produktversion `0.95.0` für `manifest.ini` und den NVDA Add-on Store.
`buildVars.development_version()` ergänzt für Entwicklungsstände eine
branchlokale SemVer-Kennung wie `0.95.0-dev.1` und nach Möglichkeit
...
- einen eindeutigen Archivnamen wie
  `NeovimAccessLink-0.95.0-dev.1+feature.example.<commit>.nvda-addon`,
- den sichtbaren Komponentenpaketnamen
  `neovim-access-link-0.95.0-dev.1+feature.example.<commit>-user.tar.gz`,
