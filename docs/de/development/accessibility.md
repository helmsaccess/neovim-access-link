...
| Gehaltene Funktions- und Diagnoseabfrage | kontextabhängig `NVDA+Leertaste` beziehungsweise `NVDA+Umschalt+Leertaste`, korrelierte Steuerantwort, öffentliche LSP- beziehungsweise `vim.diagnostic`-API; bei Funktionsname oder öffnender Aufrufklammer liegt nur die LSP-Abfrageposition vorübergehend hinter der Klammer, die schließende Klammer ist bereits eine Position innerhalb der Argumentliste | exakte Control-/Instanzbindung, Anfrage-ID, Buffer/Fenster/Tab, `changedtick`, Modus und echter Cursor; dynamische Gestenübernahme nur bei passender Fähigkeit, sonst nativer NVDA-Rückfall; begrenzte Signaturen, je Signatur unabhängige Parameterauswahl und Diagnosen | flüchtige Sprache/Braille; Erstansage und `NVDA+k/j` zeigen ausschließlich Signatur und Dokumentation, der erste `NVDA+h/l`-Befehl blendet den ausgewählten Parameter ein und erst weitere Befehle wechseln ihn; innerhalb der Meldung blätterbare Brailleansicht; Loslassen stellt die Editorbraillezeile wieder her | ja | Validator-, Protokoll-, Lua-, echter LSP-, Controller-, AppModule- und gebauter Add-on-Test einschließlich aktiver und fremder Kontexte, NVDA-Rückfall, Eingabehilfe, Abfragen auf Name sowie beiden Klammern, wiederholter Abfrage, unabhängiger Parameterstände und begrenzter Braille-Seitennavigation; praktische Windows-/NVDA-Abnahme offen |
...
| allgemeine Diagnostics | `vim.diagnostic`, `DiagnosticChanged`, native Diagnostic-Navigation und fünf Access-Link-Befehle | Quelle beziehungsweise Namespace, Schwere, optionaler Code, Bytebereich, Text, Index und Anzahl | vollständige Diagnose in Sprache/Braille; getrennte VS-Code-Earcons beim gezielten Betreten einer Fehler-/Warnungszeile und an jeder durch ausdrückliche Navigation erreichten Position in einem exakten Bereich; eigener kurzer Bestätigungsklang nur bei einer ausdrücklichen Abfrage ohne Treffer; Tippen, passive fehlerfreie Bewegung, Informationen und Hinweise stumm | ja | gehärteter Vertrag, Navigation, Speech und Klang-Gates; reale gepinnte nvim-lint-/ALE-Läufe für C, Python, Bash, Go, Rust, Ruby und Markdown sowie none-ls-LSP-Brücke auf Neovim 0.10.1/0.12.3; Windows/NVDA-Praxis offen |
...
| Verbindungs-Earcons | erster authentifizierter `fullState` einer Verbindungsinstanz sowie echte Trennung einer zuvor verbundenen fokussierten Instanz | Instanz- und Transportlaufzeit, vorheriger Verbindungszustand, ohne Textinhalt | NVDA `connected.wav` höchstens einmal je verbundener Transportlaufzeit; NVDA `disconnected.wav` genau beim echten Verlust; beide unabhängig vom Editormodus | ja | gebautes Add-on; anfängliche und wiederholte Trennung, Synchronisierung, Trennung/Wiederverbindung und Abgrenzung von Fokus-/Modusklängen automatisiert |
...
| Completion-Menü | Lebenszyklus und Auswahl per `CompleteChanged`/`CompleteDonePre`/`InsertLeave`; bei fehlender Dokumentation ein zusätzlicher öffentlicher `completionItem/resolve`-Aufruf für den ausgewählten ursprünglichen LSP-Kandidaten | ausgewählter Kandidat, Index, Anzahl, lokalisierter Typ, Parameter, Quelle und Dokumentation; eigener Zustand ohne Auswahl bei weiterhin offenem Menü | Sprache, Braille, NVDA-Vorschlagsklänge; ausdrückliche Meldung des auswahlfreien Originaltextzustands; stille Aktualisierung nachgeladener Dokumentation | ja | echtes TUI, Auswahl jenseits von Eintrag 200, Auswahlaufhebung, UTF-8-Grenzen und alle 25 LSP-Typen; Resolve-Vertrag und Abbruch listenerfrei auf Neovim 0.10.1/0.12.3, echte Pyright-Antwort separat bestätigt, erneute Windows/NVDA-Abnahme offen |
...
| Automatischer LSP-Parameter | `InsertEnter`, `CursorMovedI`, `TextChangedI`, `CompleteDone`; begrenzter Aufrufresolver und 120-ms-Bündelung | innerster Aufruf, vom Server gewählte Signatur und aktiver Parameter; exakte Buffer-/Fenster-/Textstand-/Cursoridentität | kurze, deduplizierte Sprache ohne Braillemeldung; Rückkehr in einen früheren Parameter spricht erneut | ja | Resolver-, Verschachtelungs-, String-/Kommentar-, UTF-8-, Trigger-/Retrigger-, Mehrclient-, Race-, Protokoll-, Transport-, Einstellungs- und Speech-Tests; geführter realer Pyright-/NVDA-Test für Wahrnehmung und Latenz |
