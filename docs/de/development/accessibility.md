...
| Gehaltene Funktions- und Diagnoseabfrage | `NVDA+Umschalt+P/E`, korrelierte Steuerantwort, öffentliche LSP- beziehungsweise `vim.diagnostic`-API | exakte Control-/Instanzbindung, Anfrage-ID, Buffer/Fenster/Tab, `changedtick`, Modus und Cursor; begrenzte Signaturen, Parameter und Diagnosen | flüchtige Sprache/Braille; lokale Auswahl mit `NVDA+h/l` für Parameter und `NVDA+k/j` für Signaturen oder Diagnosen; Loslassen stellt die Editorbraillezeile wieder her | ja | Validator-, Protokoll-, Lua-, Controller-, AppModule- und gebauter Add-on-Test; praktische Windows-/NVDA-Abnahme offen |
