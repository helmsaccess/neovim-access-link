...
| Dateimanager | netrw-Dateisystemdaten beziehungsweise öffentliche Plugin-APIs und Ereignisse | UTF-8-sicher bytebegrenzter Name, Pfad, Typ, getrennte Markierungs-/Copy-/Cut-Zustände, Baumstatus; typisiertes Aktionsresultat nur aus belegtem Abschluss | Sprache und Braille, einschließlich Zustandsänderung am selben Eintrag und kompakter Aktion | netrw, Oil, nvim-tree, Neo-tree, mini.files | echtes netrw, API-/Ereignisattrappen, Deduplizierung, inaktive Ziele, Erfolg/Oil-Fehler/Bündelung/Pfadminimierung sowie Zwei-/Vierbyte-, Grenz- und ungültige UTF-8-Fälle; reale Plugin-/Promptaktionen und Hardware-Braille offen |
...
| Commandline | `ModeChanged`, `CmdlineChanged`/`CmdlineLeave`, `msg_show` | Typ (`:`, `/`, `?`), Inhalt, UTF-8-Befehlszeilenposition, Modus, Bufferstatus, UI-Meldung | eigener Ton, Modus, vollständiges NVDA-Zeichen-/Wortecho, Fehler und nachfolgende gewöhnliche Meldungen in Sprache/Braille; Bufferwechsel-Rückkehrsprache mit Zielausgabe zusammengefasst | teilweise; Pager-Sonderfälle offen | gebauter Add-on- und echter TUI-Test |
