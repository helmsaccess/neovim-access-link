...
| Verbindungs-Earcon | erster authentifizierter `fullState` einer Verbindungsinstanz, erneut nach echter Transporttrennung | Instanz- und Transportlaufzeit, ohne Textinhalt | NVDA `connected.wav`, unabhängig vom Editormodus und höchstens einmal je verbundener Transportlaufzeit | ja | gebautes Add-on; wiederholte Synchronisierung, Trennung/Wiederverbindung und Abgrenzung von Fokus-/Modusklängen automatisiert |
