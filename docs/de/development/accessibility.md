...
| Gehaltene Funktions- und Diagnoseabfrage | `NVDA+Umschalt+P/E`, korrelierte Steuerantwort, öffentliche LSP- beziehungsweise `vim.diagnostic`-API; bei einem Funktionsnamen vor `(` liegt nur die LSP-Abfrageposition vorübergehend hinter der Klammer | exakte Control-/Instanzbindung, Anfrage-ID, Buffer/Fenster/Tab, `changedtick`, Modus und echter Cursor; begrenzte Signaturen, je Signatur unabhängige Parameterauswahl und Diagnosen | flüchtige Sprache/Braille; Erstansage und `NVDA+k/j` zeigen ausschließlich Signatur und Dokumentation, der erste `NVDA+h/l`-Befehl blendet den ausgewählten Parameter ein und erst weitere Befehle wechseln ihn; innerhalb der Meldung blätterbare Brailleansicht; Loslassen stellt die Editorbraillezeile wieder her | ja | Validator-, Protokoll-, Lua-, echter LSP-, Controller-, AppModule- und gebauter Add-on-Test einschließlich wiederholter Abfrage, unabhängiger Parameterstände und begrenzter Braille-Seitennavigation; praktische Windows-/NVDA-Abnahme offen |
