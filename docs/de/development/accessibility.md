...
| Gehaltene Funktions- und Diagnoseabfrage | `NVDA+Umschalt+P/E`, korrelierte Steuerantwort, öffentliche LSP- beziehungsweise `vim.diagnostic`-API; bei einem Funktionsnamen vor `(` liegt nur die LSP-Abfrageposition vorübergehend hinter der Klammer | exakte Control-/Instanzbindung, Anfrage-ID, Buffer/Fenster/Tab, `changedtick`, Modus und echter Cursor; begrenzte Signaturen, Parameter und Diagnosen | flüchtige Sprache/Braille; Erstansage in der Reihenfolge Signatur, Parameter, Dokumentation; lokale Auswahl mit `NVDA+h/l` für Parameter und `NVDA+k/j` für Signaturen oder Diagnosen; getrennte, innerhalb der Meldung blätterbare Brailleabschnitte; Loslassen stellt die Editorbraillezeile wieder her | ja | Validator-, Protokoll-, Lua-, echter LSP-, Controller-, AppModule- und gebauter Add-on-Test einschließlich wiederholter Abfrage und begrenzter Braille-Seitennavigation; praktische Windows-/NVDA-Abnahme offen |
