...
This appendix preserves fine-grained development and test-build entries
through August 5, 2026. The maintained compact [changelog](changelog.md)
summarizes current and released versions. New intermediate builds are not
appended individually; Git retains their complete history.

## 0.97.0 development through August 5, 2026
...
## Summary of pre-beta development

- The product moved to protocol v2 with SSH stdio for Linux and local RPC
  bound strictly to `127.0.0.1` for Windows. Earlier general TCP, tunnel,
  token, and v1 paths were removed.
- The add-on installs bridge, protocol, MessagePack, plugin, and assignment
  configuration rootlessly from the embedded package.
- Connection profiles and runtime instances were separated so multiple local
  and remote sessions, accounts, tabs, windows, and tmux contexts can run in
  parallel through explicit F12 assignment.
- Windows Terminal events, gestures, and suppression moved into the NVDA
  AppModule. Other applications remain unaffected, and failure restores native
  terminal output fail-open.
- Structured speech and Braille expanded across modes, navigation, editing,
  selection, completion, menus, search, diagnostics, folds, marks, registers,
  macros, and common file managers.
- Settings use NVDA configuration profiles. Component and connection dialogs
  remain accessible and do not block NVDA's main thread.
