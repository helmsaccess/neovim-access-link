...
from nvim_nvda_core import (
    ...
    local_windows_target, remote_ssh_target,
)
...
def add_remote(manager, target_id, session_id, label, client):
...
class ConnectionInstanceManagerTests(unittest.TestCase):
    def test_same_profile_and_session_can_have_parallel_distinct_instances(self) -> None:
        ...
        first = add_remote(manager, "example-host-eh", "4711", "Example host one", first_client)
        second = add_remote(manager, "example-host-eh", "4711", "Example host two", second_client)
    ...
    def test_explicit_terminal_bindings_do_not_guess_or_cross(self) -> None:
        manager = ConnectionInstanceManager()
        first = add_remote(manager, "example-host-eh", "111", "Example host eh", FakeClient())
        second = add_remote(manager, "example-host-root", "222", "Example host root", FakeClient())
    ...
    def test_runtime_ids_distinguish_tabs_sharing_process_and_window(self) -> None:
        manager = ConnectionInstanceManager()
        first = add_remote(manager, "one", "1", "First", FakeClient())
        ...
        self.assertFalse(hasattr(second, "profile_id"))
    ...
    def test_remove_and_stop_all_close_clients_and_clear_bindings(self) -> None:
        ...
        first_client = FakeClient()
        second_client = FakeClient()
        first = add_remote(manager, "one", "1", "One", first_client)
        second = add_remote(manager, "two", "2", "Two", second_client)
    ...
    def test_failed_start_is_not_retained(self) -> None:
        ...
        with self.assertRaisesRegex(RuntimeError, "start failed"):
            add_remote(manager, "one", "1", "One", FakeClient(fail_start=True))
        self.assertEqual([], manager.list())

    def test_detach_clears_bindings_without_blocking_on_client_stop(self) -> None:
        manager = ConnectionInstanceManager()
        client = FakeClient()
        instance = add_remote(manager, "one", "1", "One", client)
        terminal = TerminalIdentity(10, 100)
    ...
    def test_untyped_target_is_rejected(self) -> None:
        manager = ConnectionInstanceManager()
        with self.assertRaisesRegex(ValueError, "typed connection target"):
            manager.add_target("fake-profile", "1", "Local", FakeClient())
