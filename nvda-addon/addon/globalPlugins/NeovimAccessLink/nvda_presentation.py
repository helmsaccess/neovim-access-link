...
class NvdaPresentation:
    ...
    def __init__(
        ...
        self.suggestion_sounds = SuggestionSoundCache(
            nvda_wave_directory, on_diagnostic=on_diagnostic,
        )
    ...
    def feedback_mode(self, key):
        ...
        local_mode = (
            feedback.get(key, self._feedback_defaults.get(key, 3))
            if key else 3
        )
        ...
        for action in actions:
            ...
            if indentation is not None:
                self.report_indentation(
                    indentation, getattr(action, "indentation_level", None),
                )
            ...
            try:
                ...
                self._diagnostic(
                    "speechError", errorType=type(error).__name__, error=str(error),
                )
    ...
    def report_indentation(self, quarter_tones, level):
        ...
        if report_mode in (2, 3) and quarter_tones <= 72:
            ...
            self._diagnostic(
                "indentationTone", quarterTones=quarter_tones,
